...
import "./interfaces/zksync/IContractDeployer.sol";
...
contract MarketFactory is CloberMarketFactory, ReentrancyGuard, RevertOnDelegateCall {
    ...
    IContractDeployer private immutable _contractDeployer;
    ...
    constructor(
        ...
        _contractDeployer = IContractDeployer(0x0000000000000000000000000000000000008006);
    }
    ...
    function _storeMarketInfo(address market, address host, MarketType marketType, uint128 a, uint128 factor) internal {
        if (host == address(0)) {
            revert Errors.CloberError(Errors.EMPTY_INPUT);
        }
    ...
    function computeTokenAddress(uint256 marketNonce) external view returns (address) {
        bytes memory creationCode = type(OrderNFT).creationCode;
        bytes32 bytecodeHash;
        assembly {
            bytecodeHash := mload(add(add(creationCode, 0x20), 36))
        }
        return
            _contractDeployer.getNewAddressCreate2(
                address(this),
                bytecodeHash,
                _calculateSalt(marketNonce),
                abi.encode(address(this), canceler)
            );
    }
